...
function buildStrengthSelect() {
  filterStrength.innerHTML =
    `<option value="">${t('all_strengths')}</option>` +
    ['清淡', '中等', '浓郁'].map(v =>
      `<option value="${v}">${tStrength(v)}</option>`
    ).join('');
}

function populateOriginFilter(posts) {
  const current = filterOrigin.value;
  const origins = [...new Set(posts.map(p => p.origin).filter(Boolean))];
  filterOrigin.innerHTML =
    `<option value="">${t('all_origins')}</option>` +
    origins.map(o => `<option value="${escHtml(o)}"${o === current ? ' selected' : ''}>${escHtml(o)}</option>`).join('');
}
...
function renderCard(post) {
  ...
  const excerpt = body.slice(0, 100) + (body.length > 100 ? '…' : '');
...
function render() {
  ...
  if (q) posts = posts.filter(p => {
    const title = localized(p.title, lang).toLowerCase();
